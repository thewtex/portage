...
src_install() {
	...
	if use X; then
		...
		newins "${FILESDIR}/freevo.desktop" freevo.desktop
	fi
	...
	# FIXME the docs are old
	rm -rf "${D}/usr/share/doc"

	dodoc ChangeLog COPYING FAQ INSTALL PKG-INFO README TODO \
...
pkg_preinst() {
	if use X; then
		local KDFREEVO=`kde-config --prefix`
		if [ "x$KDFREEVO" != "x" ]; then
			...
			if [ "x${FREEVOSESSION}" != "x" ]; then
				cp "${FILESDIR}/freevo.desktop" "${D}/${FREEVOSESSION}/"
			fi
		fi
	fi
