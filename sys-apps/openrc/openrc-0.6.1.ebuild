...
SRC_URI="http://roy.marples.name/downloads/openrc/openrc-0.6.1.tar.bz2 http://www.funtoo.org/archive/corenetwork/corenetwork-1.0.tar.bz2"
...
src_install() {
	...
	rm -f $D/etc/init.d/network || die "couldn't remove network script"
	rm -f $D/usr/share/openrc/runlevels/boot/network || die "couldn't remove network link"
	rm -f $D/etc/conf.d/network || die "couldn't remove conf.d network script"

	# Install funtoo networking parts:

	cd ${WORKDIR}/corenetwork-*/init.d || die
	exeinto /etc/init.d || die
	doexe netif.tmpl netif.lo || die
	cp -a ${WORKDIR}/corenetwork-*/netif.d ${D}/etc || die
