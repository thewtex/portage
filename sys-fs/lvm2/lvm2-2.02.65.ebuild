...
src_prepare() {
	cat $FILESDIR/${PV}/patches/lvm2-2.02.65-libdir.patch | patch -p1 || die "patch failed"
	eautoreconf
}
...
src_install() {
	...
	do
		doins "${FILESDIR}/${PV}/${addon}.sh" || die
	done
	...
	do
		newinitd "${FILESDIR}/${PV}/${rc}.rc" ${rc} || die
		if [ -e "${FILESDIR}/${PV}/${rc}.confd" ] 
		then
			newconfd "${FILESDIR}/${PV}/${rc}.confd" ${rc} || die
		fi
	done
